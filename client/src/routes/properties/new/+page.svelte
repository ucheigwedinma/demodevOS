<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      const result = await api.post<{ id: number }>("/properties/", payload);
      toast.success("Property created", `"${form.name}" has been added to your portfolio`);
      goto(`/properties/${result.id}`);
    } catch (err) {
      if (err instanceof ApiError) {
        errors = err.fieldErrors;
        toast.error("Validation error", "Please fix the highlighted fields below");
      } else {
        toast.error("Something went wrong", "Could not create the property");
      }
    }
    saving = false;
  }
...
<div class="max-w-2xl">
  <Breadcrumb items={[{ label: "Properties", href: "/properties" }, { label: "New Property" }]} />
  <h1 class="text-2xl font-bold text-neutral-900 mt-3 mb-8">New Property</h1>
  ...
  <form onsubmit={handleSubmit} class="space-y-6">
    ...
    <div class="flex gap-3">
      ...
      <a href="/properties" class="px-6 py-2.5 border border-neutral-200 rounded-lg text-sm font-medium text-neutral-600 hover:bg-neutral-50 transition-colors">
        Cancel
      </a>
    </div>
  </form>
