<script lang="ts">
  ...
  import type { AllowanceListItem, AllowanceType, AllowanceFrequency, PaginatedResponse } from "$lib/types";
  import DateInput from "$lib/components/DateInput.svelte";

  let items = $state<AllowanceListItem[]>([]);
  ...
  let form = $state({
    employee: "",
    allowance_type: "other" as AllowanceType,
    name: "",
    amount: "",
  ...
  const typeLabels: Record<AllowanceType, string> = { housing: "Housing", transport: "Transport", meal: "Meal", phone: "Phone", medical: "Medical", education: "Education", other: "Other" };
  ...
  async function fetchItems() {
    ...
    try {
      ...
      if (filterType) params.allowance_type = filterType;
      if (filterActive) params.is_active = filterActive;
      const res = await api.get<PaginatedResponse<AllowanceListItem>>("/hr/allowances/", params);
      items = res.results;
  ...
  function resetForm() {
    form = { employee: "", allowance_type: "other", name: "", amount: "", frequency: "monthly", start_date: "", end_date: "", is_taxable: true, is_active: true };
    fieldErrors = {}; editingId = null;
  }
  function openCreate() { resetForm(); showSlideOver = true; }

  async function openEdit(item: AllowanceListItem) {
    resetForm(); editingId = item.id;
    try {
      const d = await api.get<any>(`/hr/allowances/${item.id}/`);
      form.employee = String(d.employee ?? "");
      form.allowance_type = d.allowance_type;
      form.name = d.name ?? "";
      form.amount = d.amount ?? "";
      form.frequency = d.frequency;
      form.start_date = d.start_date ?? "";
      form.end_date = d.end_date ?? "";
      form.is_taxable = d.is_taxable ?? true;
  ...
  async function handleSave() {
    ...
    try {
      const payload: Record<string, unknown> = { allowance_type: form.allowance_type, name: form.name, frequency: form.frequency, is_taxable: form.is_taxable, is_active: form.is_active };
      ...
      if (isEditing) { await api.patch(`/hr/allowances/${editingId}/`, payload); toast.success("Allowance updated"); }
      else { await api.post("/hr/allowances/", payload); toast.success("Allowance created"); }
  ...
  async function handleDelete(item: AllowanceListItem) {
    if (!confirm(`Delete allowance "${item.name}" for ${item.employee_name}?`)) return;
    deletingId = item.id;
    try {
      await api.delete(`/hr/allowances/${item.id}/`);
      toast.success("Allowance deleted");
      if (items.length === 1 && currentPage > 1) currentPage -= 1;
      await fetchItems();
    } catch {
      toast.error("Delete failed", "Could not delete allowance.");
    } finally {
      deletingId = null;
    }
  }
  ...
  const DEV_SAMPLES = [
    { employee: "1", allowance_type: "housing" as AllowanceType, name: "Housing Allowance", amount: "5000", frequency: "monthly" as AllowanceFrequency, start_date: "2026-01-01", end_date: "", is_taxable: false, is_active: true },
    { employee: "1", allowance_type: "transport" as AllowanceType, name: "Site Transport Allowance", amount: "1500", frequency: "monthly" as AllowanceFrequency, start_date: "2026-01-01", end_date: "", is_taxable: false, is_active: true },
    { employee: "1", allowance_type: "education" as AllowanceType, name: "Education Support", amount: "12000", frequency: "annually" as AllowanceFrequency, start_date: "2026-01-01", end_date: "2026-12-31", is_taxable: true, is_active: true },
  ];
  let devFillIdx = 0;
  function devFill() {
...
<div class="space-y-4 overflow-x-clip">
  <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
      <p class="text-[11px] font-semibold uppercase tracking-[0.35em] text-indigo-600">
        ...
        <span class="text-indigo-600">Allowances</span>
      </p>
      <h1 class="mt-2 text-2xl font-bold tracking-wide text-neutral-800">Allowances</h1>
      <p class="mt-1 max-w-2xl text-sm text-neutral-500">Manage employee allowances and benefits.</p>
    </div>
  ...
  <div class="flex flex-wrap items-center gap-3 mb-6">
    <div class="relative flex-1 max-w-sm">
      ...
      <input type="text" placeholder="Search allowances..." value={searchQuery} oninput={(e) => handleSearch(e.currentTarget.value)} class="w-full pl-9 pr-4 py-2 text-sm border border-neutral-200 rounded-lg bg-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent" />
    </div>
  ...
  {:else if items.length === 0}
    <div class="text-center py-24"><p class="text-sm text-neutral-500">{searchQuery || filterType || filterActive ? "No allowances match your filters" : "No allowances yet"}</p></div>
  {:else}
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-neutral-200 bg-neutral-50/50">
            ...
            <th class="text-center px-5 py-3 font-medium text-neutral-500 text-xs uppercase tracking-wider">Taxable</th>
        ...
        <tbody class="divide-y divide-neutral-100">
          {#each items as item}
            <tr class="hover:bg-neutral-50 transition-colors">
              ...
              <td class="px-5 py-3.5 text-neutral-600">{typeLabels[item.allowance_type]}</td>
              ...
              <td class="px-5 py-3.5 text-neutral-500 text-center">{item.is_taxable ? "Yes" : "No"}</td>
    ...
    {#if totalPages > 1}
      <div class="flex items-center justify-between mt-4 text-sm text-neutral-500">
        <span>{totalCount} allowance{totalCount !== 1 ? "s" : ""}</span>
  ...
  <div class="fixed inset-0 z-50 flex justify-end">
    ...
    <div class="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
      <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-200">
        <h2 class="text-lg font-bold text-neutral-900">{isEditing ? "Edit Allowance" : "New Allowance"}</h2>
      ...
      <div class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="al-type" class="block text-sm font-medium text-neutral-700 mb-1">Type</label>
            <select id="al-type" bind:value={form.allowance_type} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900">
              {#each Object.entries(typeLabels) as [k, v]}<option value={k}>{v}</option>{/each}
            </select>
          </div>
          <div>
            <label for="al-freq" class="block text-sm font-medium text-neutral-700 mb-1">Frequency</label>
            <select id="al-freq" bind:value={form.frequency} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900">
              {#each Object.entries(freqLabels) as [k, v]}<option value={k}>{v}</option>{/each}
            </select>
          </div>
        </div>
        <div>
          <label for="al-name" class="block text-sm font-medium text-neutral-700 mb-1">Name <span class="text-red-500">*</span></label>
          <input id="al-name" type="text" bind:value={form.name} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 {fieldError('name') ? 'border-red-400' : ''}" placeholder="e.g. Housing Allowance" />
          {#if fieldError("name")}<p class="text-xs text-red-500 mt-1">{fieldError("name")}</p>{/if}
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="al-amt" class="block text-sm font-medium text-neutral-700 mb-1">Amount <span class="text-red-500">*</span></label>
            <input id="al-amt" type="number" step="0.01" bind:value={form.amount} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 tabular-nums {fieldError('amount') ? 'border-red-400' : ''}" />
            {#if fieldError("amount")}<p class="text-xs text-red-500 mt-1">{fieldError("amount")}</p>{/if}
          </div>
          <div>
            <label for="al-curr" class="block text-sm font-medium text-neutral-700 mb-1">Currency</label>
            <p id="al-curr" class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg bg-neutral-50 text-neutral-700">{currency.config.code}</p>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="al-start" class="block text-sm font-medium text-neutral-700 mb-1">Start Date</label>
            <DateInput id="al-start" bind:value={form.start_date} />
          </div>
          <div>
            <label for="al-end" class="block text-sm font-medium text-neutral-700 mb-1">End Date</label>
            <DateInput id="al-end" bind:value={form.end_date} />
          </div>
        </div>
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            <input id="al-taxable" type="checkbox" bind:checked={form.is_taxable} class="rounded border-neutral-300" />
            <label for="al-taxable" class="text-sm text-neutral-700">Taxable</label>
          </div>
          <div class="flex items-center gap-2">
            <input id="al-active" type="checkbox" bind:checked={form.is_active} class="rounded border-neutral-300" />
            <label for="al-active" class="text-sm text-neutral-700">Active</label>
          </div>
        </div>
      </div>
