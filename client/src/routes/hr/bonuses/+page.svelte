<script lang="ts">
  ...
  import type { BonusListItem, BonusType, BonusStatus, PaginatedResponse } from "$lib/types";
  import EmployeeSelect from "$lib/components/EmployeeSelect.svelte";
  import DateInput from "$lib/components/DateInput.svelte";

  let items = $state<BonusListItem[]>([]);
  ...
  const typeLabels: Record<BonusType, string> = { performance: "Performance", annual: "Annual", signing: "Signing", referral: "Referral", project: "Project", other: "Other" };
  const statusLabels: Record<BonusStatus, string> = { pending: "Pending", approved: "Approved", paid: "Paid", cancelled: "Cancelled" };
  ...
  async function fetchItems() {
    ...
    try {
      ...
      if (filterType) params.bonus_type = filterType;
      if (filterStatus) params.status = filterStatus;
      const res = await api.get<PaginatedResponse<BonusListItem>>("/hr/bonuses/", params);
      items = res.results;
  ...
  function resetForm() {
    form = { employee: "", bonus_type: "other", amount: "", date: "", reason: "", status: "pending" };
    fieldErrors = {}; editingId = null;
  }
  function openCreate() { resetForm(); showSlideOver = true; }

  async function openEdit(item: BonusListItem) {
    resetForm(); editingId = item.id;
    try {
      const d = await api.get<any>(`/hr/bonuses/${item.id}/`);
      form.employee = String(d.employee ?? "");
      form.bonus_type = d.bonus_type;
  ...
  async function handleSave() {
    ...
    try {
      const payload: Record<string, unknown> = { bonus_type: form.bonus_type, reason: form.reason, status: form.status };
      ...
      if (isEditing) { await api.patch(`/hr/bonuses/${editingId}/`, payload); toast.success("Bonus updated"); }
      else { await api.post("/hr/bonuses/", payload); toast.success("Bonus created"); }
  ...
  async function handleDelete(item: BonusListItem) {
    if (!confirm(`Delete bonus record for ${item.employee_name}?`)) return;
    deletingId = item.id;
    try {
      await api.delete(`/hr/bonuses/${item.id}/`);
      toast.success("Bonus deleted");
      if (items.length === 1 && currentPage > 1) currentPage -= 1;
      await fetchItems();
    } catch {
      toast.error("Delete failed", "Could not delete bonus.");
    } finally {
      deletingId = null;
    }
  }
  ...
  const DEV_SAMPLES = [
    { employee: "1", bonus_type: "performance" as BonusType, amount: "15000", date: "2026-03-15", reason: "Outstanding Q4 2025 project delivery — Al Wasl Tower handed over ahead of schedule", status: "approved" as BonusStatus },
    { employee: "1", bonus_type: "project" as BonusType, amount: "8500", date: "2026-02-28", reason: "Successful handover of Marina Heights Phase 2 — zero defects at DLP inspection", status: "paid" as BonusStatus },
    { employee: "1", bonus_type: "signing" as BonusType, amount: "25000", date: "2026-01-10", reason: "Senior Quantity Surveyor recruitment — competitive market retention package", status: "pending" as BonusStatus },
  ];
  let devFillIdx = 0;
  function devFill() {
...
<div class="space-y-4 overflow-x-clip">
  <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
      <p class="text-[11px] font-semibold uppercase tracking-[0.35em] text-indigo-600">
        ...
        <span class="text-indigo-600">Bonuses & Adjustments</span>
      </p>
      <h1 class="mt-2 text-2xl font-bold tracking-wide text-neutral-800">Bonuses & Adjustments</h1>
      <p class="mt-1 max-w-2xl text-sm text-neutral-500">Track and manage bonus payments and adjustments.</p>
    </div>
  ...
  <div class="flex flex-wrap items-center gap-3 mb-6">
    <div class="relative flex-1 max-w-sm">
      ...
      <input type="text" placeholder="Search bonuses..." value={searchQuery} oninput={(e) => handleSearch(e.currentTarget.value)} class="w-full pl-9 pr-4 py-2 text-sm border border-neutral-200 rounded-lg bg-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent" />
    </div>
  ...
  {:else if items.length === 0}
    <div class="text-center py-24"><p class="text-sm text-neutral-500">{searchQuery || filterType || filterStatus ? "No bonuses match your filters" : "No bonuses yet"}</p></div>
  {:else}
        ...
        <tbody class="divide-y divide-neutral-100">
          {#each items as item}
            <tr class="hover:bg-neutral-50 transition-colors">
              ...
              <td class="px-5 py-3.5 text-neutral-600">{typeLabels[item.bonus_type]}</td>
              <td class="px-5 py-3.5 text-neutral-900 text-right tabular-nums font-medium">{currency.config.code} {item.amount}</td>
    ...
    {#if totalPages > 1}
      <div class="flex items-center justify-between mt-4 text-sm text-neutral-500">
        <span>{totalCount} bonus{totalCount !== 1 ? "es" : ""}</span>
  ...
  <div class="fixed inset-0 z-50 flex justify-end">
    ...
    <div class="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
      <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-200">
        <h2 class="text-lg font-bold text-neutral-900">{isEditing ? "Edit Bonus" : "New Bonus"}</h2>
      ...
      <div class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        {#if !isEditing}
          <div>
            <label for="bn-emp" class="block text-sm font-medium text-neutral-700 mb-1">Employee <span class="text-red-500">*</span></label>
            <EmployeeSelect id="bn-emp" bind:value={form.employee} hasError={!!fieldError('employee')} />
            {#if fieldError("employee")}<p class="text-xs text-red-500 mt-1">{fieldError("employee")}</p>{/if}
          </div>
        {/if}
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="bn-type" class="block text-sm font-medium text-neutral-700 mb-1">Bonus Type</label>
            <select id="bn-type" bind:value={form.bonus_type} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900">
              {#each Object.entries(typeLabels) as [k, v]}<option value={k}>{v}</option>{/each}
            </select>
          </div>
          <div>
            <label for="bn-status" class="block text-sm font-medium text-neutral-700 mb-1">Status</label>
            <select id="bn-status" bind:value={form.status} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900">
              {#each Object.entries(statusLabels) as [k, v]}<option value={k}>{v}</option>{/each}
            </select>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="bn-amt" class="block text-sm font-medium text-neutral-700 mb-1">Amount <span class="text-red-500">*</span></label>
            <input id="bn-amt" type="number" step="0.01" bind:value={form.amount} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 tabular-nums {fieldError('amount') ? 'border-red-400' : ''}" />
            {#if fieldError("amount")}<p class="text-xs text-red-500 mt-1">{fieldError("amount")}</p>{/if}
          </div>
          <div>
            <label for="bn-date" class="block text-sm font-medium text-neutral-700 mb-1">Date <span class="text-red-500">*</span></label>
            <DateInput id="bn-date" bind:value={form.date} />
            {#if fieldError("date")}<p class="text-xs text-red-500 mt-1">{fieldError("date")}</p>{/if}
          </div>
        </div>
        <div>
          <label for="bn-reason" class="block text-sm font-medium text-neutral-700 mb-1">Reason</label>
          <textarea id="bn-reason" bind:value={form.reason} rows={3} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none"></textarea>
        </div>
      </div>
