<script lang="ts">
  ...
  import type { DocumentCollectionListItem, DocumentCollectionStatus, PaginatedResponse } from "$lib/types";
  import DateInput from "$lib/components/DateInput.svelte";

  let items = $state<DocumentCollectionListItem[]>([]);
  ...
  let form = $state({ employee: "", document_name: "", description: "", status: "pending" as DocumentCollectionStatus, due_date: "", submitted_date: "", rejection_reason: "" });

  let totalPages = $derived(Math.max(1, Math.ceil(totalCount / pageSize)));
  let isEditing = $derived(editingId !== null);

  const statusLabels: Record<DocumentCollectionStatus, string> = { pending: "Pending", submitted: "Submitted", verified: "Verified", rejected: "Rejected" };
  ...
  async function fetchItems() {
    ...
    try {
      ...
      const res = await api.get<PaginatedResponse<DocumentCollectionListItem>>("/hr/document-collection/", params);
      items = res.results;
      totalCount = res.count;
    } catch { items = []; totalCount = 0; } finally { loading = false; }
  }

  function resetForm() { form = { employee: "", document_name: "", description: "", status: "pending", due_date: "", submitted_date: "", rejection_reason: "" }; fieldErrors = {}; editingId = null; }
  function openCreate() { resetForm(); showSlideOver = true; }

  async function openEdit(item: DocumentCollectionListItem) {
    resetForm();
    editingId = item.id;
    try {
      const d = await api.get<any>(`/hr/document-collection/${item.id}/`);
      form.employee = String(d.employee ?? "");
      form.document_name = d.document_name;
      form.description = d.description;
      form.status = d.status;
      form.due_date = d.due_date ?? "";
      form.submitted_date = d.submitted_date ?? "";
      form.rejection_reason = d.rejection_reason;
    } catch { /* ignore */ }
    showSlideOver = true;
  }
  ...
  async function handleSave() {
    ...
    try {
      const payload: Record<string, unknown> = { document_name: form.document_name, description: form.description, status: form.status, rejection_reason: form.rejection_reason };
      ...
      if (isEditing) { await api.patch(`/hr/document-collection/${editingId}/`, payload); toast.success("Document request updated"); }
      else { await api.post("/hr/document-collection/", payload); toast.success("Document request created"); }
...
<div class="max-w-7xl mx-auto">
  <div class="flex items-center justify-between mb-8">
    <div>
      <h1 class="text-2xl font-bold text-neutral-900 tracking-tight">Document Collection</h1>
      <p class="mt-1 text-sm text-neutral-500">Track documents required from new hires</p>
    </div>
  ...
  {:else if items.length === 0}
    <div class="text-center py-24"><p class="text-sm text-neutral-500">{searchQuery || filterStatus ? "No documents match your filters" : "No document requests yet"}</p></div>
  {:else}
    <div class="bg-white border border-neutral-200 rounded-xl overflow-hidden">
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-neutral-200 bg-neutral-50/50">
            <th class="text-left px-5 py-3 font-medium text-neutral-500 text-xs uppercase tracking-wider">Document</th>
            ...
            <th class="text-left px-5 py-3 font-medium text-neutral-500 text-xs uppercase tracking-wider">Submitted</th>
            <th class="text-left px-5 py-3 font-medium text-neutral-500 text-xs uppercase tracking-wider">Verified By</th>
          ...
          {#each items as item}
            <tr class="hover:bg-neutral-50 transition-colors cursor-pointer" onclick={() => openEdit(item)}>
              <td class="px-5 py-3.5">
                <p class="font-medium text-neutral-900">{item.document_name}</p>
              ...
              <td class="px-5 py-3.5 text-neutral-600 text-xs">{item.submitted_date ?? "\u2014"}</td>
              <td class="px-5 py-3.5 text-neutral-600">{item.verified_by_name ?? "\u2014"}</td>
  ...
  <div class="fixed inset-0 z-50 flex justify-end">
    ...
    <div class="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
      <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-200">
        <h2 class="text-lg font-bold text-neutral-900">{isEditing ? "Edit Request" : "Request Document"}</h2>
      ...
      <div class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        <div>
          <label for="doc-name" class="block text-sm font-medium text-neutral-700 mb-1">Document Name <span class="text-red-500">*</span></label>
          <input id="doc-name" type="text" bind:value={form.document_name} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 {fieldError('document_name') ? 'border-red-400' : ''}" />
          {#if fieldError("document_name")}<p class="text-xs text-red-500 mt-1">{fieldError("document_name")}</p>{/if}
        </div>
        {#if !isEditing}
          <div>
            <label for="doc-emp" class="block text-sm font-medium text-neutral-700 mb-1">Employee <span class="text-red-500">*</span></label>
            <EmployeeSelect id="doc-emp" bind:value={form.employee} hasError={!!fieldError('employee')} />
            {#if fieldError("employee")}<p class="text-xs text-red-500 mt-1">{fieldError("employee")}</p>{/if}
          </div>
        {/if}
        <div>
          <label for="doc-desc" class="block text-sm font-medium text-neutral-700 mb-1">Description</label>
          <textarea id="doc-desc" bind:value={form.description} rows={2} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none"></textarea>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label for="doc-status" class="block text-sm font-medium text-neutral-700 mb-1">Status</label>
            <select id="doc-status" bind:value={form.status} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900">
              {#each Object.entries(statusLabels) as [k, v]}<option value={k}>{v}</option>{/each}
            </select>
          </div>
          <div>
            <label for="doc-due" class="block text-sm font-medium text-neutral-700 mb-1">Due Date</label>
            <DateInput id="doc-due" bind:value={form.due_date} />
          </div>
        </div>
        {#if form.status === "rejected"}
          <div>
            <label for="doc-reject" class="block text-sm font-medium text-neutral-700 mb-1">Rejection Reason</label>
            <textarea id="doc-reject" bind:value={form.rejection_reason} rows={2} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none"></textarea>
          </div>
        {/if}
      </div>
