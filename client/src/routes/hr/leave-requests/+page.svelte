<script lang="ts">
  ...
  import type { LeaveRequestListItem, LeaveRequestStatus, PaginatedResponse } from "$lib/types";
  ...
  const statusLabels: Record<LeaveRequestStatus, string> = { draft: "Draft", pending: "Pending", approved: "Approved", rejected: "Rejected", cancelled: "Cancelled" };
  ...
  async function fetchItems() {
    ...
    try {
      ...
      const res = await api.get<PaginatedResponse<LeaveRequestListItem>>("/hr/leave-requests/", params);
      items = res.results;
  ...
  function resetForm() {
    form = { employee: "", leave_type: "", start_date: "", end_date: "", total_days: "", is_half_day: false, reason: "", status: "draft" };
    fieldErrors = {}; editingId = null;
  }
  function openCreate() { resetForm(); showSlideOver = true; }

  async function openEdit(item: LeaveRequestListItem) {
    resetForm(); editingId = item.id;
    try {
      const d = await api.get<any>(`/hr/leave-requests/${item.id}/`);
      form.employee = String(d.employee ?? "");
      form.leave_type = String(d.leave_type ?? "");
      form.start_date = d.start_date ?? "";
      form.end_date = d.end_date ?? "";
      form.total_days = d.total_days ?? "";
      form.is_half_day = d.is_half_day ?? false;
  ...
  async function handleSave() {
    ...
    try {
      const payload: Record<string, unknown> = { is_half_day: form.is_half_day, reason: form.reason, status: form.status };
      ...
      if (form.total_days) payload.total_days = form.total_days;
      if (isEditing) { await api.patch(`/hr/leave-requests/${editingId}/`, payload); toast.success("Leave request updated"); }
      else { await api.post("/hr/leave-requests/", payload); toast.success("Leave request created"); }
...
<div class="max-w-7xl mx-auto">
  <div class="flex items-center justify-between mb-8">
    <div>
      <h1 class="text-2xl font-bold text-neutral-900 tracking-tight">Leave Requests</h1>
      <p class="mt-1 text-sm text-neutral-500">Employee leave and absence requests</p>
    </div>
  ...
  <div class="flex flex-wrap items-center gap-3 mb-6">
    <div class="relative flex-1 max-w-sm">
      ...
      <input type="text" placeholder="Search leave requests..." value={searchQuery} oninput={(e) => handleSearch(e.currentTarget.value)} class="w-full pl-9 pr-4 py-2 text-sm border border-neutral-200 rounded-lg bg-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent" />
    </div>
  ...
  {:else if items.length === 0}
    <div class="text-center py-24"><p class="text-sm text-neutral-500">{searchQuery || filterStatus ? "No requests match your filters" : "No leave requests yet"}</p></div>
  {:else}
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-neutral-200 bg-neutral-50/50">
            ...
            <th class="text-left px-5 py-3 font-medium text-neutral-500 text-xs uppercase tracking-wider">Reviewed By</th>
        ...
        <tbody class="divide-y divide-neutral-100">
          {#each items as item}
            <tr class="hover:bg-neutral-50 transition-colors cursor-pointer" onclick={() => openEdit(item)}>
              ...
              <td class="px-5 py-3.5 text-neutral-500">{item.reviewed_by_name ?? "\u2014"}</td>
  ...
  <div class="fixed inset-0 z-50 flex justify-end">
    ...
    <div class="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
      <div class="flex items-center justify-between px-6 py-4 border-b border-neutral-200">
        <h2 class="text-lg font-bold text-neutral-900">{isEditing ? "Edit Request" : "New Leave Request"}</h2>
      ...
      <div class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        {#if !isEditing}
          <div>
            <label for="lr-emp" class="block text-sm font-medium text-neutral-700 mb-1">Employee <span class="text-red-500">*</span></label>
            <EmployeeSelect id="lr-emp" bind:value={form.employee} hasError={!!fieldError('employee')} />
            {#if fieldError("employee")}<p class="text-xs text-red-500 mt-1">{fieldError("employee")}</p>{/if}
          </div>
        {/if}
        <div>
          <label for="lr-type" class="block text-sm font-medium text-neutral-700 mb-1">Leave Type ID <span class="text-red-500">*</span></label>
          <input id="lr-type" type="number" bind:value={form.leave_type} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 {fieldError('leave_type') ? 'border-red-400' : ''}" />
          {#if fieldError("leave_type")}<p class="text-xs text-red-500 mt-1">{fieldError("leave_type")}</p>{/if}
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="lr-start" class="block text-sm font-medium text-neutral-700 mb-1">Start Date <span class="text-red-500">*</span></label>
            <DateInput id="lr-start" bind:value={form.start_date} />
            {#if fieldError("start_date")}<p class="text-xs text-red-500 mt-1">{fieldError("start_date")}</p>{/if}
          </div>
          <div>
            <label for="lr-end" class="block text-sm font-medium text-neutral-700 mb-1">End Date <span class="text-red-500">*</span></label>
            <DateInput id="lr-end" bind:value={form.end_date} />
            {#if fieldError("end_date")}<p class="text-xs text-red-500 mt-1">{fieldError("end_date")}</p>{/if}
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="lr-days" class="block text-sm font-medium text-neutral-700 mb-1">Total Days <span class="text-red-500">*</span></label>
            <input id="lr-days" type="number" step="0.5" bind:value={form.total_days} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 {fieldError('total_days') ? 'border-red-400' : ''}" />
            {#if fieldError("total_days")}<p class="text-xs text-red-500 mt-1">{fieldError("total_days")}</p>{/if}
          </div>
          <div>
        ...
        <div class="flex items-center gap-2">
          <input id="lr-half" type="checkbox" bind:checked={form.is_half_day} class="rounded border-neutral-300" />
          <label for="lr-half" class="text-sm font-medium text-neutral-700">Half Day</label>
        </div>
        <div>
          <label for="lr-reason" class="block text-sm font-medium text-neutral-700 mb-1">Reason</label>
          <textarea id="lr-reason" bind:value={form.reason} rows={3} class="w-full px-3 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none"></textarea>
        </div>
      </div>
